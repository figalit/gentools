...
//#define MYPRIME 234202403
//#define MYPRIME 143017103
//#define MYPRIME 40197601
//#define MYPRIME 50153
//#define MYPRIME 2100629
//#define MYPRIME 202823
//#define MYPRIME 1201483
#define MYPRIME 3451477
...
int main(int argc, char **argv){
  ...
  for (i=0;i<nseq;i++){
    if (names[i] != NULL && !doclean && mark[i] == 1 && !dosingle){
      fprintf(out, "@%s/1\n%s\n+\n%s\n", names[i], forwards[i], quals[i]);
      fprintf(out, "@%s/2\n%s\n+\n%s\n", names[i], reverses[i], rquals[i]);
    }
    else if (names[i] != NULL && doclean && mark[i] == 0 && !dosingle){
      if (forwards[i]!=0)
	fprintf(out, "@%s/1\n%s\n+\n%s\n", names[i], forwards[i], quals[i]);
      if (reverses[i]!=0)
	fprintf(out, "@%s/2\n%s\n+\n%s\n", names[i], reverses[i], rquals[i]);
    }
    else if (names[i] != NULL && !doclean && mark[i] == 1 && dosingle){
      if (forwards[i]!=0)
	fprintf(out, "@%s/1\n%s\n+\n%s\n", names[i], forwards[i], quals[i]);
      if (reverses[i]!=0)
	fprintf(out, "@%s/2\n%s\n+\n%s\n", names[i], reverses[i], rquals[i]);
...
int readFastq(FILE *fastqFile, int hashtype){
  ...
  */
  ...
  if (!GZ){
    ...
    while (!gzeof(fastqFile)){
      if (gzeof(fastqFile)) break;
      
      gzgets(fastqFile, line, 1000);
      if (gzeof(fastqFile)) break;
      sscanf(line, "@%s\n", myname);
