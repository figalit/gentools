...
/*
  perfect window matching, no window counting
...
void concatLocations(struct sloc **, struct tloc *, int);
...
int search_n_dump(struct trie *, char *, char [], FILE *);
void insert_tloc(struct tloc **, int, char *);
int do_file(char *, struct trie *);
struct sloc *copyLocation(struct tloc *, int);
...
int main(int argc, char **argv){
  ...
  printf("Size of treenode: %d\nSize of tlocnode: %d\n", sizeof(struct trie), sizeof(struct tloc));
...
void insert(struct trie *root, char *window, int start, char *seqname){
  ...
  insert_tloc(&(current->location), start, seqname);
...
int search_n_dump(struct trie *root, char *window, char fosmidName[], FILE *out){
  int i;
  int loc;
  struct trie *current = root;
  struct tloc *location;
  ...
  while (location != NULL){
    ...
    if (maxStart > 0) {
      ...
      fprintf(out, "%10s\t%10d\t%10d\t%6d\t%6d\t%d\t\tR\t%s\t", fosmidName, (-1*maxStart), (-1*maxStart+WINDOW_SIZE-1), maxWinStart, maxWinEnd, maxWin, DBNAME);
      fprintf(out, "\n");
    }
...
void alloc_tloc(struct tloc **loc){
  (*loc) = (struct tloc *) mymalloc (sizeof(struct tloc));
  if (*loc == NULL){
    fprintf(stderr, "Insufficient memory\n");
    exit(0);
  }
  allocs++;
  slocmem+=sizeof(struct tloc);
  (*loc)->next = NULL;
  (*loc)->start = 0;
  //(*loc)->rcstart = 0;
}

void insert_tloc(struct tloc **location, int start, char *seqname){
  ...
  /* 
     ...
     I DONT NEED TO KEEP IT SORTED FOR PFASTER

  */

  struct tloc *new;
  struct tloc *pos;
  struct tloc *posparent;
...
struct sloc *copyLocation(struct tloc *source, int windowID){
  struct sloc *new;
  struct tloc *current;
  struct sloc *ret;
  struct sloc *this;
...
void concatLocations(struct sloc **s1, struct tloc *s2, int windowID){
  struct sloc *new;
  struct sloc *s1t;
  struct tloc *s2t;
  struct sloc *s1p;
...
void *threadSearch(void *parm){
  ...
  while (readFosmid(fp, fosmid, fosmidName, qp, fqual)){
	...
	//search_n_dump(root, window, fosmidName, output);	
      }
      else{
	// some chars lost
	qflen=qflen-(skipN-i);
  ...
  while(loc != endloc){
	...
	*/
	if (((current->members->windowID - windows->windowID) >= SLIDE_SIZE)  &&  
	    ...
	    &&    (abs((current->start - loc->start)-(current->members->windowID - windows->windowID)) <= SLIDE_SIZE )  ){
	  if (mindist > (current->start - loc->start) && current->nmember > wincnt){
	    mindist = current->start - loc->start;
	    bestfit = current;
	  }
	  
	  if (current->nmember > wincnt && (current->start - loc->start)>=SLIDE_SIZE){
	    mindist = current->start - loc->start;
	    bestfit = current;
...
#ifdef DEBUG
    printf("BEFORE IF, minwin=%d\t current=%d\n",minwin, current->nmember);
#endif
