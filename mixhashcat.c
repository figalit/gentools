...
int readTwoFasta(FILE *, FILE *, int);
...
int main(int argc, char **argv){
  ...
  nseq = readTwoFasta(in, rin, hashtype);

  if (out == NULL)
    return;

  for (i=0;i<nseq;i++){
    if (names[i]!=0 && forwards[i]!=0 && reverses[i]!=0 && names[i][0]!=0 && forwards[i][0]!=0 && reverses[i][0]!=0)
      fprintf(out, "%s\t%s\t%s\n", names[i], forwards[i], reverses[i]);
  }
...
int readTwoFasta(FILE *fastaFile, FILE *revFile, int hashtype){
  int cnt;
  char ch; 
  int i,j;
  ...
  fprintf(stderr, "Counting reverse sequences.\n");
  cnt = 0; i=0; seqlen=0;
  rewind(revFile);
  while (fscanf(revFile, "%c", &ch) > 0){
    if (ch == '>'){
      fscanf(revFile, "%s", dummy);
      if (strlen(dummy)>maxnamelen)
	maxnamelen=strlen(dummy);
      if (seqlen>maxlen)
	maxlen=seqlen;
      cnt++;
      seqlen=0;
      my_fgets(dummy, SEQ_LENGTH, revFile);
    }
    else if (!isspace(ch))
      seqlen++;
  }
  ...
  fprintf(stderr, "LEFT: %d\tRIGHT: %d\tTOTAL: %d\n", seqcnt, rseqcnt, clonecnt);
  ...
  */
  ...
  thisrevseq = (char *) mymalloc(maxlen*2);
  ...
  fprintf(stderr, "Reading reverse sequences.\n");
  rewind(revFile);
  cnt = -1; 
  ...
  while (fscanf(revFile, "%c", &ch) > 0){
    if (ch == '>'){
      cnt++;
      my_fgets(myname, SEQ_LENGTH, revFile);
      myname[strlen(myname)-1] = 0;
