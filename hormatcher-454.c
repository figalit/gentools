...
double percenthamming(char *S, char *T){
  int i;
  int similar=0;
  int length;
  int prev=0;
...
int readFasta(FILE *alnFile, FILE *horFile){
  ...
  printf("[OK] %d HOR sequences read\n",horseqcnt);
...
int main(int argc, char **argv){
  ...
  if (argc != 9){
    printf("Usage: %s -m [fastaFile] -h [hor file] -d [maxdist] -o [outdir]\n", argv[0]);
    exit(0);
  }

  monofile = (char *)malloc(200);
  horfilename = (char *)malloc(200);

  for (i=1;i<argc;i++){
    if (!strcmp(argv[i], "-m"))
      strcpy(monofile, argv[i+1]);
    else if (!strcmp(argv[i], "-h"))
      strcpy(horfilename, argv[i+1]);
  ...
  fasta = fopen(monofile, "r");
  if (fasta == NULL){
    printf("no fasta %s\n", monofile);
    exit(0);
  }
  horfile = fopen(horfilename, "r");
  ...
  printf("MONOFILE %s\nHORFILE %s\nDISTANCE %d\nOUTDIR %s\n", monofile, horfilename, mismatch,outdir);
  ...
  */
  ...
  for (i=strlen(monofile)-1;i>=0;i--)
    if (monofile[i]=='/') break;
  monofile+=i+1;

  for (i=strlen(horfilename)-1;i>=0;i--)
    if (horfilename[i]=='/') break;
  horfilename+=i+1;
  

  sprintf(fname, "%s/%s-%s.m%d.matrix", outdir, monofile, horfilename, mismatch);
  printf("outfasta %s\n", fname);
  outfasta = fopen(fname, "w");
  if (outfasta==NULL)
    printf("no outfasta\n");

  sprintf(str, "%s/%s-%s.m%d.histogram", outdir, monofile,horfilename, mismatch);
  histogram = fopen(str, "w");
  if (histogram==NULL)
    printf("no histogram\n");

  sprintf(str, "%s/%s-%s.m%d.log", outdir, monofile,horfilename, mismatch);
  log = fopen(str, "w");
  if (log==NULL)
    printf("no log\n");
  ...
  */
  ...
  freqs = (int *) malloc(sizeof(int)*horcnt);
  for (i=0;i<horcnt;i++){
    freqs[i] = 0;
  }
  ...
  */
  ...
  fwrite(&monocnt, sizeof(int), 1, outfasta);
  fwrite(&horcnt, sizeof(int), 1, outfasta);
  ...
  for (j=0; j<horcnt; j++){
    ...
    for (i=0; i<monocnt; i++){
      ...
      //revcomp
      rcomp(horseqs[j], revstr);
      score = pairwise(seqs[i], revstr);
      rdistance=hamming(aligned[0], aligned[1]);

      if (distance <= mismatch){
        fprintf(log, "%s\tFORWARD\n", names[i]);
        count++;
      }
      
      else if (rdistance <= mismatch){
	// log negative distance for REVERSE matches
	fprintf(log, "%s\tREVERSE\n", names[i]);
	count++;
      }
      
    
      rdistance = rdistance * -1;
      fwrite(&distance, sizeof(short), 1, outfasta);    
      fwrite(&rdistance, sizeof(short), 1, outfasta);    
  ...
  */
  ...
  fprintf(plotfile, "set output \"%s-%s.histogram.eps\"\n",monofile,horfilename);
  ...
  fprintf(plotfile, "plot \"%s-%s.histogram\" with boxes\n", monofile, horfilename);
  fclose(histogram);
  fclose(plotfile);
  system("gnuplot plot");
  sprintf(str, "%s-%s.plotfile", monofile, horfilename);
  rename("plot", str);
  printf("\n");
  */


  fclose(log); fclose(outfasta);
