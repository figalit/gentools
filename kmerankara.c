...
#define FOSMID_LENGTH 260000000
...
int main(int argc, char **argv){
  ...
  /*  sprintf(command, "sort -n -k 2,2 %s.index | tail -n %d > top%d.%s", idxname, HISTSTRETCH, HISTSTRETCH, idxname);
  system(command);
  */
  /*
  sprintf(command, "echo \"-- 0\" >> top10.%s", idxname);
  system(command);
  */
  /*
  sprintf(command, "sort -n -k 2,2 %s.index | head -n %d | sort -nr -k 2,2 >> top%d.%s", idxname, HISTSTRETCH, HISTSTRETCH, idxname);
  system(command);
  */
  /*
  sprintf(command, "awk \'{print $2}\' top10.%s > freq10.%s", idxname, idxname);
  system(command);
  */


  /*
  sprintf(command, "sort -nr -k 2,2 %s.index > top%d.%s", idxname, HISTSTRETCH, idxname);
  system(command);

  sprintf(fgnuplot, "%s.gnuplot", idxname);

  gnuplot = fopen(fgnuplot, "w");

  sprintf(fgnuplot, "top%d.%s", HISTSTRETCH, idxname);
  freqFile = fopen(fgnuplot, "r");
  sprintf(fgnuplot, "freq%d.%s", HISTSTRETCH, idxname);
  freqList = fopen(fgnuplot, "w");

  fprintf(gnuplot, "set terminal postscript eps enhanced color\n");
  fprintf(gnuplot, "set output \"top%d.%s.eps\"\n", HISTSTRETCH, idxname);
  
  fprintf(gnuplot, "set title \"Comparison of %s and %s\"\n", fname, fname2);
  fprintf(gnuplot, "set xlabel \"k-mer\"\n");
  fprintf(gnuplot, "set ylabel \"delta_freq\"\n");
  //fprintf(gnuplot, "set size 2,2\n");
  fprintf(gnuplot, "unset xtics\n");
  

  i = 0;
  j = 0;
  while (fscanf(freqFile, "%s %d", gnuseq, &gnufreq) > 0){
    if (gnufreq >= 0){
      if (i<HISTSTRETCH){
	fprintf(gnuplot, "set label \"%s\" at first %d,0 rotate by -90 font \"Times,12\" \n", gnuseq, i);
	fprintf(freqList, "%d\n", gnufreq);
	j++;
      }
    }
  ...
  sprintf(command, "sort -n -k 2,2 %s.index > top%d.%s", idxname, HISTSTRETCH, idxname);
  system(command);
  sprintf(fgnuplot, "top%d.%s", HISTSTRETCH, idxname);
  freqFile = fopen(fgnuplot, "r");
  */
  ...
  fclose(freqList);
  fprintf(gnuplot, "plot \"freq%d.%s\" with boxes\n", HISTSTRETCH, idxname);
  
  fclose(gnuplot);

  sprintf(command, "gnuplot %s.gnuplot", idxname); 
...
int do_file(char *fname, struct trie *root, int which){
  ...
  while (readFosmid(fasta, genome, dummy)){
    glen = strlen(genome);
    cnt++;
    ...
    willInsert = getWindow(genome, glen, &window, i);
    while (willInsert >=0){
      if (VERBOSE)
	printf("%s\n", window);
