...
void findPositions(struct sloc *, char *, int, FILE *, int);
...
void insertCluster(struct cluster **, struct sloc *, struct sloc *, char *, FILE *, int, int);
...
void *threadSearch(void *parm){
  ...
  while (readFosmid(fp, fosmid, fosmidName)){
    ...
    if (locations!=NULL){
      findPositions(locations, fosmidName, qflen, output, minwin);
...
void findPositions(struct sloc *locations, char *fosmidName, int flen, FILE *out, int minwin){
  struct sloc *current;
  struct sloc *this;
  struct sloc *last;
  int thisStart;
...
#endif
	  insertCluster(&cRoot, this, last->next, fosmidName, out, minwin, flen);
	  dumpstart = this->start;
	  ////dumpend = last->end;
	  free_cluster(cRoot);
	}
	else if (count >= minwin && last==NULL){
	  insertCluster(&cRoot, this, NULL, fosmidName, out, minwin, flen);
	  dumpstart = this->start;
	  //dumpend = this->end;
	  free_cluster(cRoot);
	}
...
void insertCluster(struct cluster **cRoot, struct sloc *newloc, struct sloc *endloc, char *fosmidName, FILE *out, int minwin, int qflen){
