...
void insert_clone(struct clonenode **, int, char *);
...
void insert_clone(struct clonenode **location, int count, char *seqname){
...
void *threadSearch(void *parm){
  ...
  while (readFosmid(fp, fosmid, fosmidName)){
    ...
    insert_clone(&clones, count, clonename);
    fprintf(output, "%s\t%d\n", fosmidName, count);
    }
