...
/*
typedef struct trie{
  void *kids[4]; //children nodes
}strie;

typedef struct trie_nonleaf{
  void *kids[4]; //nonleaf nodes have only children
}strie_nonleaf;

typedef struct trie_leaf{
  struct sloc *location; // leaf nodes do not have children
}strie_leaf;
*/
...
void findPositions(struct sloc *, char *, int, int, FILE *, int, char *, int);
...
void insert_sloc(struct sloc **, int);
...
void insertCluster(struct cluster **, struct sloc *, struct sloc *, char *, FILE *, int, int, int, char *, int);
...
void insert_sloc(struct sloc **location, int start){
...
void dumpLocation(struct trie *root, FILE *fp, unsigned int location_count){
  ...
  //if (!TEXTFORMAT) {
    ...
    // ONUR: Eee... filesize'i o kadar kucultunce % 30 oluyor integer'in katkisi :-)
...
void loadTrie(char *fname, struct trie **root){
  ...
  while ((read_bytes = fread(encoded_window, sizeof(unsigned char) * encoded_window_bytes, 1, fp))) {
    ...
    */
    ...
    while (i) {
      ...
      insert_sloc(&locs, start); // ONURNEW
  ...
  /*
    ...
    start = atoi(dummy);
    start += INDEXSTART;
    insert(*root, window, start); // #0: INSERT + strand
...
void findPositions(struct sloc *locations, char *fosmidName, int flen, int qflen, FILE *out, int minwin, char *fosmid, int orient){
  struct sloc *current;
  struct sloc *this;
  struct sloc *last;
  int thisStart;
...
void alloc_cluster(struct cluster **cl){
  ...
  (*cl)->members = NULL;
  (*cl)->next = NULL;
  (*cl)->nmember = 0; /* CALKAN : THIS SHOULD BE 1. LEFT 0 FOR NOW FOR DEBUG COMPARISON */
  clallocs++;
}
...
void insertCluster(struct cluster **cRoot, struct sloc *newloc, struct sloc *endloc, char *fosmidName, FILE *out, int minwin, int flen, int qflen, char *fosmid, int orient){
