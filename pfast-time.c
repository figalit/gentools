...
int main(int argc, char **argv){
  ...
  if (!LOADED){
    ...
    gettimeofday(&end, &tz);
    loadtime = ((int)(end.tv_sec*1000000+end.tv_usec)-(int)(start.tv_sec*1000000+start.tv_usec));
  }
  ...
  fprintf(stdout, "load time %ld microseconds\n", loadtime);
  fprintf(stdout, "fosmidread time %ld microseconds\n", fosmidtime);
  fprintf(stdout, "write time %ld microseconds\n", writetime);
...
int readFosmid(FILE *fp, char *fosmid, char *name, FILE *qp, int *qual){
  ...
  gettimeofday(&end, &tz);
  fosmidtime +=  ((int)(end.tv_sec*1000000+end.tv_usec)-(int)(start.tv_sec*1000000+start.tv_usec));
  pthread_mutex_unlock(&readMutex);  	
...
void insertCluster(struct cluster **cRoot, struct winloc *newloc, struct winloc *endloc, char *fosmidName, FILE *out, int minwin, int flen, int qflen, char *fosmid, int orient){
  ...
  while (current != NULL){
	...
	writetime +=  ((int)(end.tv_sec*1000000+end.tv_usec)-(int)(start.tv_sec*1000000+start.tv_usec));
