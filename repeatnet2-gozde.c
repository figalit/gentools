...
void compare(char *fname, int hashtype, float ecut, int ccut, int acut, int merge){
  ...
  // NOTE: THE FILE HEADER CHANGED BY 4 BYTES AS OF AUG 22, 2007
...
int loadMatrix(char *fname, char *fname2){
  ...
  while (!feof(matlog)){
    ...
    // TODO: implement a matrix data structure to store i,j,eboth
...
void connectedComponent(int matrixcnt, int wincnt){
  struct Component *components;
  struct Component *ccmp;
  struct Node *newnode;
  FILE *matlog;
  FILE *matviz;
  FILE *mattxt;
  ...
  // the matrix is dumped starting node 0 to 4^WS (if exists)
  // and loaded the same way
  int current=1;
  int v1, v2, w; //vertex1, vertex2, weight;
  int ncomponents;
  int dumped;
  ...
  // push first; loop the rest

  v1 = interactions[0].i;
  v2 = interactions[0].j;
  w = interactions[0].eweight;
  
  //alloc_component(&components);
  insert_node_to_component(&components, v1, v2, w);
  windowflag[v1] = 1;
  windowflag[v2] = 1;

  for (i=1;i<matrixcnt;i++){
    v1 = interactions[i].i;
    v2 = interactions[i].j;
    w = interactions[i].eweight;
    // think think think
    // keep the component id?? in windowflag
    insert_node_to_component(&components, v1, v2, w);
    //getchar();
  }
  ...
  /* this is probably OK, try to dump matrices */ 
  /* CALKAN: NOT OK, CHECK CHECK CHECK */
  ...
  while(ccmp != NULL){
    ...
    while(newnode != NULL){
      ...
      fwrite(&(newnode->v1), sizeof(int), 1, matlog);
      fwrite(&(newnode->v2), sizeof(int), 1, matlog);
      fwrite(&(newnode->w), sizeof(int), 1, matlog);    
      
      decode_window(newnode->v1, window1);
      decode_window(newnode->v2, window2);
      fprintf(mattxt, "%s\n%s\n", window1, window2);
      fprintf(matviz, "\t%s\t--\t%s [ color=\"%s\" ];\n", window1, window2, colors[ncomponents%10]);
...
void insert_node_to_component(struct Component **cmp, int v1, int v2, int w){

  struct Node *new;
  struct Component *newcmp;
  struct Component *prev;
  struct Component *prevcmp;
  int compid;
  int movecomp;
  int i;
  int small, large;
  ...
  compid = 0;  /* start component ids from 1 */
  
  newcmp = (*cmp);
 
  movecomp = 0;

  printf("%d:%d\t%d:%d\n", v1, windowflag[v1], v2, windowflag[v2]);

  if (windowflag[v1] != 0) movecomp = windowflag[v1];
  else if (windowflag[v2] != 0) movecomp = windowflag[v2];

  if (windowflag[v1] != windowflag[v2] && windowflag[v1] !=0 && windowflag[v2] !=0){
    /*merge*/
    printf("merging %d:%d %d:%d\n", v1, windowflag[v1], v2, windowflag[v2]);
    //getchar();
    compid = windowflag[v1];
    
    if (movecomp < compid){
      small = movecomp; large=compid;
    }
    else{
      small = compid; large=movecomp;
    }
    ...
    assert(prevcmp != NULL);
    assert(newcmp != NULL);
    prevcmp->next = newcmp->next;
    movecomp = windowflag[v2];
    for(i=1;i<small;i++)
      prev = prev->next;
    new = newcmp->node;
    while (new->next != NULL){
      printf("update %d:%d\t%d:%d\twith %d\n", new->v1, windowflag[new->v1], new->v2, windowflag[new->v2], small);
      windowflag[new->v1] = small;
      windowflag[new->v2] = small;
      new = new->next;
    }
    assert(new!=NULL);
    assert(prev!=NULL);
    new->next = prev->node;
    prev->node = newcmp->node;
    newcmp = (*cmp); 
    windowflag[v1] = small;
    windowflag[v2] = small;
  }
  ...
  if (newcmp == NULL){
    alloc_component(&newcmp);
    assert(newcmp!=NULL); 
  }

  if (newcmp->node == NULL){
    alloc_node(&(newcmp->node));
    newcmp->node->v1 = v1;
    newcmp->node->v2 = v2;
    newcmp->node->w = w;
	...
	movecomp++;
      ...
      prev->next = newcmp;    
      windowflag[v1] = movecomp;
      windowflag[v2] = movecomp;
    }
    //printf("inserting\t%d\t%d\t%d\tto\t%d\n", v1,v2,w, windowflag[v1]);
    //getchar();
    return;
  }
  ...
  if ((*cmp) == NULL){
    ...
    //printf("movecmp 0 for %d-%d-%d\n", v1, v2, w);
    //insert newcmp at the end
    movecomp = 1;
    prev = (*cmp);
    assert((*cmp)!=NULL); 
    alloc_component(&newcmp);
