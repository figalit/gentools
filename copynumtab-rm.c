...
void dochr(char *thischr, FILE *border, FILE *acgh, FILE *rep);
...
int main(int argc, char **argv){
  ...
  for (i=1;i<=22;i++){
    ...
    fprintf(stderr, "-->%s\n", thischr);
    rewind(border); rewind(acgh); rewind(rep);
    dochr(thischr, border, acgh, rep);
  }
...
void dochr(char *thischr, FILE *border, FILE *acgh, FILE *rep){
  ...
  while (fscanf(acgh, "%s\t%d\t%d\n", chr, &s, &e) > 0){
    if (strcmp(chr, thischr)) continue;
    for (i=s; i<=e; i++) seq[i] = -999;
  }
  
  
  fprintf(stderr, "Copy number table is loaded, reading BED file\n");
  ...
  while (fscanf(border, "%s\t%d\t%d\n", chr, &s, &e) > 0){
    ...
    if (total != 0)
      fprintf(dump, "%s\t%d\t%d\t\t%d\t%f\t%f\n", chr, s, e, count, thiswin[count/2], (total/count));
    else
      fprintf(dump, "%s\t%d\t%d\t\t%d\t%f\t%f\n", chr, s, e, count, 0, 0);
