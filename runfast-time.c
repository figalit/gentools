...
int main(int argc, char **argv){
  ...
  while(fscanf(in, "%s%s%d", index, seqname, &start) > 0){
    if (minwin != 0)
    sprintf(command, "pfast-time -loadindex %s -search %s -istart %d -threads %d -seqname %s -minwin %d", index, sfile, start, threads, seqname, minwin);
    else
    sprintf(command, "pfast-time -loadindex %s -search %s -istart %d -threads %d -seqname %s > %s.log", index, sfile, start, threads, seqname, index);
    system(command);
  }
...
//  system("gzip *out");
//  system(command);
  sprintf(command, "mv *out results.out.txt pruned.out.txt %s.pfast", sfile);
  system(command);
