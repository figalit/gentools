...
int main (int argc, char **argv) {
	...
	else if (!strcmp(argv[i], "-infile") || !strcmp(argv[i], "-i") ) {
	...
	if (!GZ) {
	    ...
	    while (!feof(input)) {
		fscanf(input, "%s\t%d\t%d\t%d\t%c\t%s", fosmidName, &chrStart, &chrEnd, &maxWin, &orientchar, DBNAME);
		...
		if (!DOC)
		  ...
		  printf("%10s\t%10d\t%10d\t%d\t%c\t%s\n", fosmidName, chrStart, chrEnd, maxWin, orientchar, DBNAME);
		else
		...
		sscanf(lineString, "%s\t%d\t%d\t%d\t%c\t%s", fosmidName, &chrStart, &chrEnd, &maxWin, &orientchar, DBNAME);
		if (!DOC)
		  //		  printf("%10s\t%10d\t%10d\t%6d\t%6d\t%d\t\t%c\t%s\n", fosmidName, chrStart, chrEnd, maxWinStart, maxWinEnd, maxWin, orientchar, DBNAME);  
		  printf("%10s\t%10d\t%10d\t%d\t%c\t%s\n", fosmidName, chrStart, chrEnd, maxWin, orientchar, DBNAME);		  
		else
