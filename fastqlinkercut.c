...
//#define MYPRIME 849801179
#define MYPRIME 40197601
//#define MYPRIME 50153
...
int main(int argc, char **argv){
  ...
  char readleft[20000];
  char qualleft[20000];
  char readright[20000];
  char qualright[20000];
  ...
  remcnt = 0;
  while(fscanf(rin, "%s %d %d", removethis, &s, &e) > 0){
    //   end2clone(removeclone, removethis);
    removeindex = hash(removethis , nseq, hashtype); // hope it works

    mark_s[removeindex] = s;
    mark_e[removeindex] = e;

    //    printf ("%d : %s - %s marked\n", removeindex, removethis, names[removeindex]);
    //getchar();
    remcnt++;
  }
  ...
  for (i=0;i<nseq;i++){
    ...
    if (names[i] != NULL && mark_s[i] != 0 && mark_e[i] != 0){
      
      /* Illuminization is default */

      k=0;
      for (j=mark_s[i]-1;j>=0;j--){
	readleft[k] = comp(forwards[i][j]);
	qualleft[k] = quals[i][j];
	k++;
      }
      readleft[k]=0; qualleft[k]=0;
      /*
      for (j=0;j<mark_s[i];j++){
	readleft[j] = forwards[i][j];
	qualleft[j] = quals[i][j];
      }
      readleft[j]=0; qualleft[j]=0;
      */
      k=0;
      for (j=strlen(forwards[i])-1;j>mark_e[i];j--){
	readright[k] = comp(forwards[i][j]);
	qualright[k] = quals[i][j];
	k++;
      }
      readright[k]=0; qualright[k]=0;      
    

    if (strlen(readleft)<36 || strlen(readright)<36) continue;

    if (ILLUMINIZE){
      readleft[37] = 0; readright[37] = 0;
      qualleft[37] = 0; qualright[37] = 0; 
    }

    
    fprintf(out, "@%s/1\n%s\n+\n%s\n@%s/2\n%s\n+\n%s\n", names[i], readleft, qualleft, names[i], readright, qualright);
    }
    /*
    else if (names[i] != NULL && doclean && mark[i] == 0){
      fprintf(out, "@%s/1\n%s\n+\n%s\n", names[i], forwards[i], quals[i]);
      fprintf(out, "@%s/2\n%s\n+\n%s\n", names[i], reverses[i], rquals[i]);
      }*/
  }
...
int readFastq(FILE *fastqFile, int hashtype){
  int cnt;
  char ch; 
  int i,j;
  ...
  while (fscanf(fastqFile, "%c", &ch) > 0){
    if (ch == '>'){
      fscanf(fastqFile, "%s", dummy);
	...
	maxlen=seqlen;
      cnt++;
      seqlen=0;
      my_fgets(dummy, SEQ_LENGTH, fastqFile);
    }
    else if (!isspace(ch))
      seqlen++;
  }
  ...
  */
  ...
  //reverses = (char **) mymalloc((clonecnt) * sizeof(char *));
  quals = (char **) mymalloc((clonecnt) * sizeof(char *));
  //rquals = (char **) mymalloc((clonecnt) * sizeof(char *));
  ...
  */
  ...
  mark_s = (char *) mymalloc((clonecnt) * sizeof(char ));
  mark_e = (char *) mymalloc((clonecnt) * sizeof(char ));

  memset(mark_s, 0, sizeof(char)*clonecnt);
  memset(mark_e, 0, sizeof(char)*clonecnt);
  ...
  */
  ...
  //  while (fscanf(fastqFile, "@%s\n%s\n%s\n%s\n", myname,  thisseq, dummy, myqual) > 0){
  while (fscanf(fastqFile, "@%s", myname) > 0){


    fgets(dummy, 1000, fastqFile);

    fscanf(fastqFile, "%s\n%s\n%s\n", thisseq, dummy, myqual);

    
    //    end2clone(myclone, myname);
    
    index = hash(myname , clonecnt, hashtype); 


    
    forwards[index] = (char *)mymalloc(sizeof(char)*(strlen(thisseq)+1));
    strcpy(forwards[index], thisseq);
    quals[index] = (char *)mymalloc(sizeof(char)*(strlen(myqual)+1));
    strcpy(quals[index], myqual);
    


    if (names[index] == NULL)
      names[index] = (char *) mymalloc(sizeof(char)*(strlen(myname)+1));
    strcpy(names[index], myname);

    //printf("%s-%s added to %d\n", myname, names[index], index); getchar();
