...
int main(){
  ...
  nc = 0;
  while (fscanf(stdin, "%s\t%d\t%d\t%s\t%d\t%f\t%d\n", chr, &s, &e, call, &span, &prob, &sup) > 0){
  ...
  while (one!=NULL){
    ...
    if (strcmp(one->call, REMOVED))
      fprintf(stdout, "%s\t%d\t%d\t%s\t%d\t%f\t%d\n", one->chr, one->s, one->e, one->call, one->span, one->prob, one->sup);
    one = one->next;
  }
...
int merge(struct sv *one, struct sv *two){
  ...
  */
  ...
  //  prob = (one->prob + two->prob) / 2;
  prob = ((one->prob * one->sup) + (two->prob * two->sup)) / (float)(one->sup + two->sup);
