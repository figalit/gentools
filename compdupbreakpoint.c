  ...
  for (i=0;i<n1;i++){
	...
	continue;
      ...
      */
      ...
      if (var1[i]->s <= var2[j]->s && var1[i]->e >= var2[j]->e){
	//if (!strcmp(var1[i]->call2, var2[j]->call2))
	/* var2 inside var1 */
	
	ratio = (float)(var2[j]->e - var2[j]->s + 1) / (float)(var1[i]->e - var1[i]->s + 1);
	
	bp1 = var1[i]->s - var2[j]->s; 
	bp2 = var1[i]->e - var2[j]->e; 
	
	//ratio=1;
	if (ratio>=0.8)
	  fprintf(stdout, "%s\t%d\t%d\t%d\t%d\t%d\t%d\t%4.2f\t%d\t%d\n", var1[i]->chr, var1[i]->s, var1[i]->e, (var1[i]->e-var1[i]->s), var2[j]->s, var2[j]->e, (var2[j]->e-var2[j]->s), ratio, bp1, bp2);
	...
	*/
      ...
      else if (var1[i]->s <= var2[j]->s && var1[i]->e <= var2[j]->e && var1[i]->e > var2[j]->s){
	//if (!strcmp(var1[i]->call2, var2[j]->call2))

	ratio = (float)(var1[i]->e - var2[j]->s + 1) / (float)(var2[j]->e - var1[i]->s + 1);

	ratio2 = (float)(var1[i]->e - var2[j]->s + 1) / (float)(var2[j]->e - var2[j]->s + 1);
	ratio3 = (float)(var1[i]->e - var2[j]->s + 1) / (float)(var1[i]->e - var1[i]->s + 1);

	bp1 = var1[i]->s - var2[j]->s; 
	bp2 = var1[i]->e - var2[j]->e; 

	if (ratio>=0.8)// || ratio2 > 0.8 || ratio3 > 0.8)
	  fprintf(stdout, "%s\t%d\t%d\t%d\t%d\t%d\t%d\t%4.2f\t%d\t%d\n", var1[i]->chr, var1[i]->s, var1[i]->e, (var1[i]->e-var1[i]->s), var2[j]->s, var2[j]->e, (var2[j]->e-var2[j]->s), ratio, bp1, bp2);
	/*
	if (!strcmp(var1[i]->call2, var2[j]->call2))
	  fprintf(stdout, "concordant\n");
