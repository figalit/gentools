...
*/
    ...
    */
    ...
    sprintf(mvop,"mv /tmp/%s/%s.map %s/", myjobid,currentfile,outdir);
		...
		if (((*myfilelist)[i].finished==0) && ((*myfilelist)[i].started==0)) //if that file hasn't been touched by other processor yet
	 	{
			...
			//sprintf(out,"%s map /tmp/%s/%s.map /tmp/PFASTDB/bacs_masked.bfa %s/%s\n%s\n%s\n%s\n",blastparameters, myjobid,currentfile, inputdir,currentfile, outputdir,myjobid,currentfile);
			sprintf(out,"%s map /tmp/%s/%s.map /var/tmp/hg17.bfa %s/%s\n%s\n%s\n%s\n",blastparameters, myjobid,currentfile, inputdir,currentfile, outputdir,myjobid,currentfile);
