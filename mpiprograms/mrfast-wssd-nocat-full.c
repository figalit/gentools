...
#include <sys/param.h>
#include <stdio.h>
#include "/net/eichler/vol6/software/mpich-1.2.7_amd64/include/mpi.h"
#include <string.h>
...
*/
    ...
    sprintf(mvop,"mkdir /tmp/%s/%s",myjobid,currentfile);
    system(mvop);
    ...
    */
    ...
    for (i=1;i<=22;i++){
      ...
      sprintf(mvop, "cat /tmp/%s/%s/chr%d_*coverage,out > /tmp/%s/%s/chr%d.coverage.out", myjobid, )
    ...
    sprintf(mvop,"mv /tmp/%s/%s %s/",myjobid,currentfile,outdir);
		...
		if (((*myfilelist)[i].finished==0) && ((*myfilelist)[i].started==0)) //if that file hasn't been touched by other processor yet
	 	{
			...
			//sprintf(mvop,"mkdir /tmp/%s/%s",myjobid,currentfile);    
			sprintf(out,"%s -search %s/%s -outfolder /tmp/%s/%s\n%s\n%s\n%s\n",blastparameters,inputdir,currentfile,myjobid, currentfile, outputdir,myjobid,currentfile);
