...
int main(int argc, char **argv){
  ...
  for (i=0;i<argc;i++){
    ...
    /*
     ...
     CHRBYCHR=1;
    */
    else if (!strcmp(argv[i], "-overmap"))
      strcpy(overmap, "-overmap");
  }
  ...
  */
  ...
  sprintf(command, "/net/eichler/vol3/home/calkan/bin/mrfast-snp-qual -loadidx %s -search %s -outfolder %s -nocnt %s %s %s %s %s", genomeidx, fullsfile, outeachfolder, textout, summary, norandom, nom, noy);
  ...
  if (!NOMATCH && CHRBYCHR){
    ...
    for (i=1;i<=22;i++){
      ...
      sprintf(command, "/net/eichler/vol3/home/calkan/bin/solexamatch-snp-qual -i %s -o %s.match -u %d -l %d -m 1000000 %s %s -min 72", outchrname, outchrname, upper, lower, best, noconcordant);
  ...
  */
  ...
  if (!NOMATCH){
    ...
    system(command);
    sprintf(command, "/net/eichler/vol3/home/calkan/bin/solexamatch-snp-qual -i %s -o %s.match -u %d -l %d -m 1000000 %s %s -min 72 -text -textsingle -gzsingle -single %s", outname, outname, upper, lower, best, noconcordant, overmap);
    printf("\nSOLEXAMATCH  command is %s\n\n", command);
    system(command);
