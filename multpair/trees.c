...
//extern float  **tmat;     /* general nxn array of reals; allocated from main */
...
static float  	*av;
static float  	*left_branch, *right_branch;
static float  	*save_left_branch, *save_right_branch;
...
void nj_tree(char **tree_description, FILE *tree)
{
	...
	float  fnseqs,fnseqs2=0,sumd;
	float  diq,djq,dij,d2r,dr,dio,djo,da;
	float  tmin,total,dmin;
	float  bi,bj,b1,b2,b3,branch[4];
...
/*********************** First initialisation ***************************/
	...
	if (fnseqs == 2) {
	  /*
	    if (verbose) fprintf(tree,"Cycle   1     =  SEQ:   1 (%9.5f) joins  SEQ:   2 (%9.5f)",tmat[first_seq+1][first_seq],tmat[first_seq+1][first_seq]);
	  */
		return;
	}
	...
	left_branch 	= (float  *) ckalloc( (nseqs+2) * sizeof (double)   );
	right_branch    = (float  *) ckalloc( (nseqs+2) * sizeof (double)   );
	tkill 		= (sint *) ckalloc( (nseqs+1) * sizeof (sint) );
	av   		= (float  *) ckalloc( (nseqs+1) * sizeof (double)   );
		...
		for(j=1; j<=last_seq-first_seq+1; ++j) 
			if( tkill[j] != 1 ) {
			  /*
				...
				else if (mini<=j && minj>j)
				  da = ( tmat[j][mini] + tmat[minj][j] ) * 0.5;
				else if (mini>j && minj<=j)
				  da = ( tmat[mini][j] + tmat[j][minj] ) * 0.5;
				else
				  da = ( tmat[j][mini] + tmat[j][minj] ) * 0.5;
				if( (mini - j) > 0 ) 
					tmat[mini][j] = da;
				if( (mini - j) < 0)
					tmat[j][mini] = da;
...
/******************************Last Cycle (3 Seqs. left)********************/
	...
	if (l[1]>l[2]) {
	  if (l[2]>l[3]){
	   ...
	   if (l[1] > l[3]){
	    b1 = (tmat[l[1]][l[2]] + tmat[l[1]][l[3]] - tmat[l[3]][l[2]]) * 0.5;
	    b2 =  tmat[l[1]][l[2]] - b1;
	    b3 =  tmat[l[1]][l[3]] - b1;
	   }
	   else{
	    b1 = (tmat[l[1]][l[2]] + tmat[l[3]][l[1]] - tmat[l[3]][l[2]]) * 0.5;
	    b2 =  tmat[l[1]][l[2]] - b1;
	    b3 =  tmat[l[3]][l[1]] - b1;
           }
          }
	  ...
	  if (l[2]<l[3]){
	   b1 = (tmat[l[2]][l[1]] + tmat[l[3]][l[1]] - tmat[l[3]][l[2]]) * 0.5;
	   b2 =  tmat[l[2]][l[1]] - b1;
	   b3 =  tmat[l[3]][l[1]] - b1;
 	  }
	  else{
	   if (l[1] > l[3]){
	    b1 = (tmat[l[2]][l[1]] + tmat[l[1]][l[3]] - tmat[l[2]][l[3]]) * 0.5;
	    b2 =  tmat[l[2]][l[1]] - b1;
	    b3 =  tmat[l[1]][l[3]] - b1;
	   }
	   else{
	    b1 = (tmat[l[2]][l[1]] + tmat[l[3]][l[1]] - tmat[l[2]][l[3]]) * 0.5;
	    b2 =  tmat[l[2]][l[1]] - b1;
	    b3 =  tmat[l[3]][l[1]] - b1;
           }
          }
...
/* save the left_branch and right_branch for phylip output */
	save_left_branch = (float  *) ckalloc( (nseqs+2) * sizeof (double)   );
	save_right_branch = (float  *) ckalloc( (nseqs+2) * sizeof (double)   );
	for (i=1;i<=nseqs;i++) {
...
void print_nexus_tree(char **tree_description, FILE *tree, sint bootstrap)
{
	...
	if(last_seq-first_seq+1==2) {
	  ; // calkan
	  //fprintf(tree,"(%d:%7.5f,%d:%7.5f);",first_seq,tmat[first_seq+1][first_seq],first_seq+1,tmat[first_seq+1][first_seq]);
	}
	else {
...
void print_phylip_tree(char **tree_description, FILE *tree, sint bootstrap)
{
	...
	for (i=1;i<=nseqs;i++){
	  for (j=1;j<=nseqs;j++)
	    printf("%d ",tree_description[i][j]);
	  printf("\n");
	}
	if(last_seq-first_seq+1==2) {
	  //fprintf(tree,"(%s:%7.5f,%s:%7.5f);",names[first_seq],tmat[first_seq+1][first_seq],names[first_seq+1],tmat[first_seq+1][first_seq]);
		return;
	}
...
*/
{
	...
	*/
	...
	if(numseqs==2) {
	  //dist=tmat[firstseq+1][firstseq]/2.0;
