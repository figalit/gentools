...
*/
	...
	params[0]=(char *)ckalloc(strlen(args[0]+2)*sizeof(char));
...
*/
#ifndef UNIX
		for(k=0;k<(sint)strlen(args[0]);++k) seqname[k]=tolower(args[0][k]);
#else
		for(k=0;k<(sint)strlen(args[0]);++k) seqname[k]=args[0][k];
#endif 
		s++;
	}
...
/**************************************************/
	...
	//	printf("SEQNAME: %s setinfile %d\n", seqname, setinfile);
	
	if (seqname[strlen(seqname)-1] < 'A' || seqname[strlen(seqname)-1] > 'z' )
...
1)) {
	...
	for (i=1;i<=new_seq;i++) {
			...
			/*
			  ...
			  tmat[j][i] = ((double)100.0 - (double)dscore)/(double)100.0;			
			//tmat[j][i] = tmat[i][j];
			*/
	/*
     		}
