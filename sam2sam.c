...
int main(int argc, char **argv){
  ...
  char rest1[1500];  char rest2[1500];
  ...
  while(!strstr(rest1, "ID:GATK")){
    fgets(rest1, 1500, stdin);
    fprintf(stdout, "%s", rest1);
  }
    ...
    */
	...
	strcpy(quality2, txtswap);
      ...
      */
      ...
      fprintf(stdout, "%s\t%d\t%s\t%d\t%d\t%s\t%s\t%d\t%d\t%s\t%s%s", name, flag, chr, start, mapqual, cigar, mrnm, mpos, isize, sequence, quality, rest1);      
    }
