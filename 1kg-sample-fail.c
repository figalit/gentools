  ...
  */
  ...
  for (i=0;i<lanecnt;i++){
	...
	maxcut = mean + 4*stdev + 1;
      ...
      coverage = ((double)numreads / 3000000000.0)*  mylanes[i].readlen;
      ...
      if (numfiles <= 300) maxcpu = numfiles+1; else if (numfiles >= 601) maxcpu = 301; else maxcpu = numfiles / 2;
      ...
      /* SAMPLE */
      	...
      	sprintf(command, "mrmap -submit -q all.q -ssh -options \"--maxoea 1000\" -email %s -g %s -e %d -pe -min 0 -max 1000 -mincpu 100 -maxcpu 101 -i %s_sample -o %s_sample -sge %s-%s-%d_sample.sge > %s-%s-%d.sampleID", EMAIL, MRSFASTDB, editdist, indir, outdir, mylanes[i].samplename, mylanes[i].libname, mylanes[i].readlen, mylanes[i].samplename, mylanes[i].libname, mylanes[i].readlen);
	...
	/*
	while (!isfinished){
	  fprintf(stderr, "Sleeping for 10 minutes.\n");
	  sleep(600);
	  printf("Checking sample map log.\n");
	  fprintf(stderr, "Checking sample map log.\n");
	  ...
	  fclose(maplog);
	    ...
	    printf("Sample job not yet started.\n");
	}

	*/

	sprintf(command, "cat %s_sample/*sam | awk \'{if ($9>0 && $9<%d) print $9}\' > %s/STATS/%s-%s-%d.len", outdir, (median*2+20), samplename, samplename, libname, readlen);
	...
	sprintf(command, "stats -i %s/STATS/%s-%s-%d.len -revert -title; gnuplot %s/STATS/%s-%s-%d.len.gnuplot", samplename, samplename, libname, readlen, samplename, samplename, libname, readlen);
	...
	sprintf(maplogfile, "%s/STATS/%s-%s-%d.len.gnuplot", samplename, samplename, libname, readlen);
	maplog = fopen(maplogfile, "r");
	...
	*/
	...
	if (strcmp(maplogfile, "")){
      ...
      sprintf(command, "rm -fr %s/%s/%d/out_sample", samplename, libname, readlen);
      printf("COMMAND: %s\n", command);
      system(command);
