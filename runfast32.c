...
int main(int argc, char **argv){
  ...
  while(fscanf(in, "%s%s%d", index, seqname, &start) > 0){
    sprintf(command, "pfast32 -loadindex %s -search %s -istart %d -threads %d -seqname %s", index, sfile, start, threads, seqname);
    system(command);
  }
  ...
  system("prunepfast32 results.out.txt pruned.out.txt");
