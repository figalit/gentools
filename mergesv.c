...
void insert_sv(struct sv **, char *, int, int, char *, int, int);
...
int main(){
  ...
  nc = 0;
  while (fscanf(stdin, "%s\t%d\t%d\t%s\t%d\t%d\n", chr, &s, &e, call, &span, &sup) > 0){
...
void insert_sv(struct sv **root, char *chr, int s, int e, char *call, int span, int sup){
