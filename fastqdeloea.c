...
//#define MYPRIME 2100629
#define MYPRIME 202823
...
int main(int argc, char **argv){
  ...
  while(fscanf(rin, "%s", removethis) > 0){
    ...
    removeindex = hash(removeclone , nseq, hashtype); // hope it works
    //printf("MARK: %s\t%d\n", removeclone, removeindex);
    if (isReverse(removethis))
      mark[removeindex] = RMARK;
    else 
      mark[removeindex] = FMARK;
    //printf ("%d : %s marked\n", removeindex, names[removeindex]);
    remcnt++;
  }
...
int readFastq(FILE *fastqFile, int hashtype){
  ...
  */
  ...
  while (fscanf(fastqFile, "@%s\n%s\n%s\n%s\n", myname,  thisseq, dummy, myqual) > 0){
    ...
    if (isReverse(myname)){
      reverses[index] = (char *)mymalloc(sizeof(char)*(strlen(thisseq)+1));
      strcpy(reverses[index], thisseq);
      rquals[index] = (char *)mymalloc(sizeof(char)*(strlen(myqual)+1));
      strcpy(rquals[index], myqual);
    }
    else{
  ...
  if (ch == ''){
  cnt++;
  my_fgets(myname, SEQ_LENGTH, fastqFile);
