...
void findPositions(struct sloc *, char *, int, FILE *, int, char *, int);
...
void insertCluster(struct cluster **, struct sloc *, struct sloc *, char *, FILE *, int, int, char *, int);
...
void *threadSearch(void *parm){
  ...
  while (readFosmid(fp, fosmid, fosmidName, qp, fqual)){
    ...
    if (locations!=NULL){
      findPositions(locations, fosmidName, qflen, output, minwin, fosmid, FORWARD);
    ...
    flen = strlen(fosmid);
    if (strlen(fosmid) != strlen(rfosmid)){
    ...
    if (locations!=NULL){
      findPositions(locations, fosmidName, qflen, output, minwin, rfosmid, REVERSE);
      freeLocation(locations);
    }
...
void findPositions(struct sloc *locations, char *fosmidName, int flen, FILE *out, int minwin, char *fosmid, int orient){
  struct sloc *current;
  struct sloc *this;
  struct sloc *last;
  int thisStart;
...
#endif
	  insertCluster(&cRoot, this, last->next, fosmidName, out, minwin, flen, fosmid, orient);
	  dumpstart = this->start;
	  ////dumpend = last->end;
	  free_cluster(cRoot);
	}
	else if (count >= minwin && last==NULL){
	  insertCluster(&cRoot, this, NULL, fosmidName, out, minwin, flen, fosmid, orient);
	  dumpstart = this->start;
	  //dumpend = this->end;
	  free_cluster(cRoot);
	}
...
void insertCluster(struct cluster **cRoot, struct sloc *newloc, struct sloc *endloc, char *fosmidName, FILE *out, int minwin, int qflen, char *fosmid, int orient){
  ...
  while (current != NULL){
	...
	if (orient == FORWARD) {
	  ...
	  /* CALKAN : CHECK THESE COORDINATES */
	  /* CALKAN : maxWinStart and maxWinEnd should switch and normalized with fosmid_length (reverse!!) */
