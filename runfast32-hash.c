...
int main(int argc, char **argv){
  ...
  while(fscanf(in, "%s%s%d", index, seqname, &start) > 0){
    ...
    if (!DOCAT){
      if (minwin!=0)
	sprintf(command, "/mnt/local/inhousebin/pfast-short -loadindex %s -search %s -istart %d -threads %d -seqname %s -outfolder %s -minwin %d -minident %f -singlefile", index, fullsfile, start, threads, seqname, outeachfolder, minwin, minident);
      else
	sprintf(command, "/mnt/local/inhousebin/pfast-short -loadindex %s -search %s -istart %d -threads %d -seqname %s -outfolder %s -minident %f -singlefile", index, fullsfile, start, threads, seqname,outeachfolder, minident);
      printf("Command is %s \n",command); 
	...
	system(command);
      ...
      if (minwin!=0)
	sprintf(command, "/mnt/local/inhousebin/pfast-short -loadindex %s -search %s -istart %d -threads %d -seqname %s -outfolder %s -minwin %d -minident %f -minlen %f -nocnt", index, fullsfile, start, threads, seqname, outeachfolder, minwin, minident, minlen);
      else
	sprintf(command, "/mnt/local/inhousebin/pfast-short -loadindex %s -search %s -istart %d -threads %d -seqname %s -outfolder %s -minident %f -minlen %f -nocnt", index, fullsfile, start, threads, seqname,outeachfolder, minident, minlen);
      printf("Command is %s \n",command); 
  ...
  */
  ...
  if (DOMATCH){
    ...
    sprintf(command, "/mnt/local/inhousebin/pfast_sort -i %s/%s.match -o %s/%s.sort -e 3000 -s 300", outeachfolder, sfile, outeachfolder, sfile); 
    printf("Command is %s \n",command);
    system(command);
