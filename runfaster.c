...
int main(int argc, char **argv){
  ...
  while(fscanf(in, "%s%s%d", index, seqname, &start) > 0){
    if (minwin != 0)
    sprintf(command, "pfaster -loadindex %s -search %s -istart %d -threads %d -seqname %s -minwin %d", index, sfile, start, threads, seqname, minwin);
    else
    sprintf(command, "pfaster -loadindex %s -search %s -istart %d -threads %d -seqname %s", index, sfile, start, threads, seqname);
    system(command);
  }
